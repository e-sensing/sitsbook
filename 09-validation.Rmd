...
With this motivation, when measuring accuracy of classified images, the function `sits_accuracy()` follows the procedure set by Olofsson et al. [@Olofsson2013]  The following explanation is extracted from thar paper, and users should refer to this paper for further explanation.
...
The first step is to obtain the classification map. Since the map is too large for sharing by an R package, the data is available in a Dropbox folder. The code below shows how to retrieve these files. 

```{r, tidy="styler", echo = FALSE, eval = TRUE}
data_dir <- "./tempdir/chp9"

# read the files from Dropbox to a local directory
files_read <- purrr::map_chr(files_drop, function (f){
    local_files 
})
