...
> Access to the Swiss Data Cube requires gdal 3.4 (see the instructions below). Note that gdal running in `docker` containers requires the security flag `--security-opt seccomp=unconfined` on start. For more information, check the [gdal mailing list](https://www.mail-archive.com/search?l=gdal-dev@lists.osgeo.org&q=subject:%22%5C%5Bgdal%5C-dev%5C%5D+%5C%2Fvsimem%5C%2F+Driver+CPLCreateUserFaultMapping+Error+in+Docker%22&o=newest&f=1).
