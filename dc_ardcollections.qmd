...
ARD (analysis-ready) image collections are organized into spatial partitions. Sentinel-2 images follow the Military Grid Reference System (MGRS) tiling system, which divides the world into 60 UTM zones of 6 degrees of longitude. Each zone is divided into latitude bands of 8 degrees. Bands are split into tiles of 110 $\times$ 110 km$^2$ with a 10 km overlap. @fig-mgrs-datacubes shows the MGRS tiling system for a part of the northeastern coast of Brazil, contained in UTM zone 24, block M. 
...
#| out-width: 80%
#| fig-cap: |
#|   WRS-2 tiling system used by Landsat images (source: USGS).
#| fig-align: center
...
- Copernicus Data Space Ecosystem - OData (CDSE-OS): Open data Sentinel-1 GRD images. 
...
The `LANDSAT-C2-L2` collection provides access to data from the Landsat-5/7/8/9 satellites. Images from these satellites have been intercalibrated to ensure data consistency. For compatibility between the different Landsat sensors, the band names are BLUE, GREEN, RED, NIR08, SWIR16, and SWIR22. All images have 30 m resolution. For this collection, tile search is not supported; the `roi` parameter should be used. The example below shows how to retrieve data from a region of interest covering the Lencois Maranhenses in the Northeastern coast of Brazil.
...
The Copernicus DEM 30-meter is organized in a 1$^\circ$ by 1$^\circ$ grid. In `sits`, access to COP-DEM-GLO-30 images can be done either by MGRS tiles (`tiles`) or by region of interest (`roi`). In both cases, the cube is retrieved based on the parts of the grid that intersect the region of interest or the chosen tiles. The resulting DEM cube
may includes more than one tile. For this reason, users need to state which specific
tile they want to visualize.
...
#| out-width: 80%
#| fig-cap: |
#|   Hierarchical BDC tiling system showing (a) large BDC grid overlayed on Brazilian biomes, (b) one large tile from the grid used for CBERS-4 AWFI data, (c) four medium tiles from the grid used for LANDSAT data; and (d) sixteen small tiles from the grid used for SENTINEL-2 data. Tiles in (b), (c), and (d) are nested.
#| fig-align: center
...
### SENTINEL-1-GRD images in CDSE

Sentinel-1 Level-1 Ground Range Detected (GRD) data in the Copernicus Data Space Ecosystem (CDSE) consists of focused C-band synthetic aperture radar imagery multi-looked and projected to ground range using the WGS84 ellipsoid. Ground range coordinates are the slant range coordinates projected onto the ellipsoid of the Earth. Pixel values represent detected magnitude. Phase information is lost. The resulting product has approximately square spatial resolution and square pixel spacing with reduced speckle due to the multi-look processing.

The CDSE Sentinel-1 GRD data is organized in a 1$^\circ$ by 1$^\circ$ grid. In `sits`, access to these images can be done either by MGRS tiles (`tiles`) or by region of interest (`roi`). In both cases, the cube is retrieved based on the parts of the grid that intersect the region of interest or the chosen tiles. The resulting cube
may includes more than one tile. For this reason, users need to state which specific
tile they want to visualize. See the example below for more details.

Unlike optical Sentinel-2 data, which is accessed via the STAC protocol using `source = "CDSE"`, Sentinel-1 RTC images are retrieved through the OpenSearch API using `source = "CDSE-OS"`. The following example shows an S1-RTC image for the Nile Delta in Egypt.

::: {.panel-tabset}
## R
```{r}
#| label: fig-s1-grd-cdse
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 5
...
#| fig-dpi: 300
#| fig-cap: |
#|   Sentinel-1-GRD image of the Nile Delta in Egypt.
#| fig-align: center
#| out-width: 80%
# retrieve a S1-RTC cube and plot
roi = c(lon_min = 31.5, lon_max = 32.5, lat_min = 31.0, lat_max = 32.0)
...
- Annual geomedian images for Landsat 8 and Landsat 9 (`GM-LS8-LS9-ANNUAL`, Landsat-8/9 OLI) in the WRS-2 grid system.
...
Both HLS and Sentinel-2 tiling systems use the MGRS tiling system. Each tile is 109.8 km wide, with 4,900 m of overlap on each side.
...
## Landsat collection from Open Geo Hub

OpenGeoHub Foundation (OGH) is a not-for-profit research foundation based in Wageningen, the Netherlands. It promotes open-source software, transparent environmental data science, and open access to global geographical and geoscientific information. OGH provides analysis-ready, cloud-optimized (ARCO) global Landsat bimonthly mosaics at a 30-meter resolution spanning from 1997 to 2024. These public datasets feature 7 spectral bands plus clear-sky masks as Cloud Optimized GeoTIFFs, accessible via STAC catalogs to support environmental monitoring and AI land-cover mapping. 

Due to technology differences among the sensors onboard different LANDSAT satellites, there are significant variations in the values obtained by each satellite for the same area on Earth’s surface, which reduces the applicability of the data for long-term time series analysis and machine learning algorithms.

Global Land Analysis and Discovery (GLAD) from University of Maryland (UMD) created a harmonized analysis-ready landsat collection (Landsat ARD) for the entire planet covering a multidecadal span of years (1997–2022+). Described in detail in [@Potapov2020], it is an important dataset for long-term monitoring of forests, croplands, snow, population dynamics, and other environmental variables at global scale. OGH adapted the GLAD data and made it available openly. 

The Landsat dataset available on OGH is global by default. Thus, when users select an area of interest, a global cube is returned. To obtain a regular data cube for selected areas, use `sits_cube_copy` to copy the desired region to a local directory.

:::{.panel-tabset}
## R
```{r}
#| label: fig-ogh-global
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 10
...
#| fig-dpi: 300
#| fig-cap: |
#|     Global plot of Landsat collection available at OGH.
#| out-width: 100%
# define a region of interest
roi <- sits_tiles_to_roi("36RUV", grid_system = "MGRS")
# define a global data cube from the OGH collection
ogh_cube <- sits_cube(
     source = "OGH",
     collection = "LANDSAT-GLAD-2M",
     bands = c("BLUE", "NIR", "SWIR2"),
     roi = roi,
...
After selecting the global cube, users can obtain data for their area of interest using `sits_cube_copy()`.

:::{.panel-tabset}
## R
```{r}
#| label: fig-ogh-nile-delta
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 5
...
#| fig-dpi: 300
#| fig-cap: |
#|     Landsat image of Nile Delta (MGRS tile 36RUV).
#| out-width: 80%

# set output dir for ROI data if it does not exist 
tempdir_r_ogh <- "~/sitsbook/tempdir/R/dc_ardcollections/ogh"
dir.create(tempdir_r_ogh, showWarnings = FALSE, recursive = TRUE)

# extract a global data cube from the OGH collection
roi_cube <- sits_cube_copy(
     cube = ogh_cube,
     roi = roi,
     output_dir = tempdir_r_ogh
)

# plot the cube
plot(roi_cube, date = "2018-03-01")
```
## Python
```{python}
#| eval: false
# set output dir for ARD data if it does not exist 
tempdir_py_ogh <- "~/sitsbook/tempdir/Python/dc_ardcollections/ogh"
dir.create(tempdir_py_ogh, showWarnings = FALSE, recursive = TRUE)
...
Terrascope is an online platform for accessing open-source satellite images. This service, operated by VITO, offers a range of free Earth observation data and processing services. Currently, `sits` supports the World Cover 2021 and World Cereal maps, produced by VITO with support from the European Commission and ESA. The following code shows how to access the World Cover 2021 covering tile "22LBL". First, we use `sits_tiles_to_roi()` to get the region of interest expressed as a bounding box, based on an MGRS tile; this box is then entered as the `roi` parameter in the `sits_cube()` function. Since the World Cover data is available as a 3$^\circ$ by 3$^\circ$ grid, it is necessary to use `sits_cube_copy()` to extract the exact MGRS tile.

To access Terrascope, users need to be registered at the [cloud provider](https://terrascope.be/en). After registration, please include the environmental variables `TERRASCOPE_USER` and `TERRASCOPE_PASSWORD` in your `.Rprofile` configuration.

:::{.panel-tabset}
## R
```{r}
#| label: fig-terrascope
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 5
...
# plot the resulting map
plot(world_cover_2021_20LBL, legend_position = "inside")
```
## Python
```{python}
#| eval: false
...
The AlphaEarth Foundations satellite embedding dataset is produced by Google and Google DeepMind [@Brown2025]. The `ALPHAEARTH` collection provides annual 64-band satellite embeddings (bands `A00` to `A63`) at 10 m resolution, covering the period from 2017 to 2025.

Unlike the ARD collections described above, whose bands correspond to physical measurements, each pixel of an AlphaEarth image is a unit-length vector in a 64-dimensional embedding space, which summarizes the temporal trajectory of surface conditions at and around that pixel over one calendar year. The model that produces these embeddings assimilates many data streams, including optical, radar, and LiDAR data, and thus the individual bands are not independently interpretable; all 64 bands should be used together in downstream analysis.

Images are organized in a dedicated grid whose tiles have 8192 $\times$ 8192 pixels, each one in its local UTM projection; either `tiles` or `roi` should be used to select the area of interest. The data is openly available under a CC-BY 4.0 license, and is distributed as cloud-optimized GeoTIFF files by [Source Cooperative](https://source.coop/tge-labs/aef), which hosts an open copy of the Google dataset.

The following example shows how to access the AlphaEarth embeddings for a region covering part of the Brazilian Federal District.

:::{.panel-tabset}
## R
```{r}
#| label: fig-alpha-earth
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 5
...
#| fig-dpi: 300
#| fig-cap: |
#|     AlphaEarth Foundations satellite embedding for a region in the Brazilian Federal District.
#| out-width: 80%
# Access AlphaEarth Foundations annual satellite embeddings
aef_cube <- sits_cube(
    source = "GOOGLE",
...
# Plot the cube
plot(aef_cube, red = "A03", green = "A01", blue = "A02")
```
## Python
```{python}
#| eval: false
