...
## Comparison of sits and Google Earth Engine APIs{-}

This section provides a comparison between the sits and Google Earth Engine APIs, 
focusing on their capabilities to perform specific tasks using satellite data. 
The comparison will be structured around three examples: (1) creating a mosaic 
(2) calculating the Normalized Difference Vegetation Index (NDVI), and (3) performing 
a land use and land cover (LULC) classification. Each section will illustrate 
how these tasks are accomplished using sits and GEE, highlighting similarities 
and differences in the API approaches.

### Example 1: Creating a Mosaic{-}

### Example 2: Calculating NDVI{-}

This example demonstrates how to generate Normalized Difference Vegetation 
Index (NDVI) using both the `sits` and `Google Earth Engine (gee)` APIs.

#### Define Region of Interest data{-}

In this example, an Region of Interest is defined using the `sinop_roi.shp` file. 
The following code blocks specify this file in the `sits` and `gee` APIs.
...
other sources are also available. For `gee`, the platform provides the data 
directly.
...
> In both APIs, defining an ROI before the calculation is possible. For this
example, the data was calculated before.
...
classification using both the `sits` and `Google Earth Engine (gee)` APIs.

#### Define Region of Interest (ROI) data{-}
...
of `18` samples. The following code snippets show how these samples are defined 
in each environment.
...
Next, we load satellite imagery. In this example, data from [MOD13Q1](https://ladsweb.modaps.eosdis.nasa.gov/missions-and-measurements/products/MOD13Q1) 
is used. In `sits`, this data is retrieved from the Brazil Data Cube, but other 
sources are also available. In `gee`, the platform directly provides this data.

In `sits`, all necessary transformations for using the data in a classification 
task are handled automatically. In `gee`, users must transform the data into the 
correct format, such as converting all images into bands for time-series 
classification using Random Forest.

> The approach adopted in `gee` for this example may require adjustments when 
working with larger datasets or more bands. In `sits`, the API usage remains 
consistent regardless of the data size.
...
To train a Random Forest classifier, we need sample data associated with 
time-series. In this step, we extract the time-series for the samples using the 
previously loaded satellite imagery.
...
Forest classifier by associating temporal behavior with labels from the samples.
...
With the trained Random Forest classifier, we can generate the classification map. 

In `sits`, this involves two steps: (1) Generating class probabilities and 
(2) Generating the label map. This method is advantageous because it allows 
further transformations, such as applying Bayesian smoothing to reduce artifacts 
in the final result.

In `gee`, the same operation is performed in a single step, but it doesn't allow 
for working with probabilities.
