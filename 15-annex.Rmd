...
satellite data. The exploration will be structured around three key examples: 
...
Each example will demonstrate how these tasks are executed using `sits` and `gee`, 
...
> To reproduce the example, you can download the shapefile using [this link](#). 
...
[samples_sinop_crop for sits](#) or [samples_sinop_crop for gee](#)
...
In `gee`, the classification is completed in a single step, and the probability 
outputs from the classifier cannot be analyzed or manipulated.
