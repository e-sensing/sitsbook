...
Except in the cases of combining DEMs with Sentinel-2 cubes, and joining Sentinel-1 and Sentinel-2, the other cases require a combination of two operations: `sits_merge()` and `sits_regularize()`. The first function combines the timelines of the data cubes, in most cases producing a non-regular data cube. For this reason, users have to use `sits_regularize()` to produce a multi-source regular cube.

## Merging HLS Landsat and Sentinel-2 collections

Images from the HLS Landsat and Sentinel-2 collections are accessed separately and can be combined with `sits_merge()`. We first create two ARD collections, one for HLS Sentinel-2 and one for HLS Landsat over the same area. The two cubes are then merged. We use the HLS collection available at Microsoft Planetary Computer ("MPC"), which has the same content as the NASA HLS collection, but provides a faster access.
...
The merged cube contains a denser timeline. However, for further use in `sits`, we need to create a regularized cube to obtain a composite with fewer clouds. To make processing easier, we copy the original data to a local directory.

:::{.panel-tabset}
## R
```{r}
#| label: merge-cube-copy-local
#| results: hide
#| warning: false
#| cache: true
...
```{r}
#| echo: false
#| label: merge-cube-local-recreate
#| results: hide
#| warning: false
#| cache: true
# define the output directory
...
# copy the hls cube for a local directory for faster regularization
cube_hls_local <- sits_cube(
    source = "MPC",
    collection = "HLSS30",
    data_dir = tempdir_r_hls
)
```
...
The next step is to create a regular data cube for tile "22LBL" and Sentinel-2 data.
...
```
:::
The next step is to create a regular Sentinel-1 data cube. The plot uses three dates, which are shown respectively in red, green and blue.
...
In many applications, especially in regions with large topographical, soil or climatic variations, it is useful to merge multitemporal data cubes with base information such as digital elevation models (DEM). Merging multitemporal satellite images with digital elevation models (DEMs) offers several advantages that enhance the analysis and interpretation of geospatial data. Elevation data provides an additional dimension to the two-dimensional satellite images, which helps to distinguish land use and land cover classes that are impacted by altitude gradients. One example is the capacity to distinguish between low-altitude and high-altitude forests. In cases where topography changes significantly, DEM information can improve the accuracy of classification algorithms.
...
After obtaining the 1$^\circ$ by 1$^\circ$ data cube covering the selected tile, the next step is to regularize it. This is done using the `sits_regularize()` function. This function will produce a DEM that matches exactly the chosen tile.
...
#| eval: false
# set output dir if it does not exist 
tempdir_py_dem_19HCC = tempdir_py / "dem_19HCC"
tempdir_py_dem_19HCC.mkdir(parents=True, exist_ok=True)
...
dem_cube_19HCC_reg = sits_regularize(
  ...
  bands = "ELEVATION",
  tiles = "19HCC",
  output_dir = tempdir_py_dem_19HCC
)
...
There are two ways to combine multitemporal data cubes with DEM data. The first method takes the DEM as a base information, which is used in combination with the multispectral time series. For example, consider a situation of a data cube with 10 bands and 23 time steps, which has a 230-dimensional space. Adding DEM as a base cube will add one dimension to the attribute space. This combination is supported by function `sits_add_base_cube()`. In the resulting cube, the information on the image time series and that of the DEM are stored separately. The data cube metadata will now include a column called `base_info`.
...
#| warning: false
#| cache: true
merged_cube_base <- sits_add_base_cube(s2_cube_19HCC_reg, dem_cube_19HCC_reg)
...
#| warning: false
#| cache: true
merged_cube <- sits_merge(s2_cube_19HCC_reg, dem_cube_19HCC_reg)
