...
The following code uses `sits_select()` to get a subset of the sample dataset with two bands (NDVI and EVI), and whose labels is `Cerrado`.
...
#| message: false
# Select NDVI band
samples_cerrado_ndvi <- sits_select(
...
#| cache: true
#| message: false
# Select NDVI band and Cerrado label
samples_cerrado_ndvi = sits_select(
    samples_matogrosso_mod13q1, 
    bands = "NDVI",
    labels = "Cerrado")
...
The default time series plot in `sits` aligns all samples onto a single reference time interval (based on the first sample) for display purposes, even when the samples' actual observations span different calendar years. This plot shows the spread of values for the time series of each band. The strong red line in the plot indicates the median of the values, while the two orange lines are the first and third quartiles. See `?sits::plot` for more details on data visualization in `sits`.

:::{.panel-tabset}
## R
```{r}
#| label: fig-samples-cerrado-ndvi
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 7
...
#| fig-align: center
#| fig-cap: | 
#|    Plot of Cerrado samples for NDVI band.
# Plot cerrado samples together
plot(samples_cerrado_ndvi)
```
## Python
```{python}
#| eval: false
...
```{r}
#| echo: false
#| label: py-fig-samples-cerrado-ndvi
#| results: hide
#| warning: false
#| cache: true
#| fig-width: 7
...
To see the spatial distribution of the samples, use `sits_view()` to create an interactive plot. The spatial visualization is useful to show where the data has been collected.
...
$$
y = \beta_{0} + f(x) + \epsilon, \epsilon \sim N(0, \sigma^2).
$$
...
#| fig-align: center
#| fig-cap: | 
#|     Patterns for Mato Grosso MOD13Q1 samples.
...
samples_modis_ndvi = sits_select(
    samples_matogrosso_mod13q1, 
    bands = "NDVI"
)
...
## Python
```{python}
#| label: py-ts-csv-samples-raster
#| cache: true
#| message: false
#| warning: false
# Obtain a raster cube based on local files
