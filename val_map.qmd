...
#| fig-dpi: 300
#| fig-cap: |
#|   Classified mosaic for land cover and land use in Rondonia state (Brazil) for 2022.
#| fig-align: center
#| out-width: 90%
...
plot(rondonia_2022_class, legend_position = "inside")
```
## Python
```{python}
#| echo: true
...
```
```{r}
#| label: py-fig-val-rondonia-class
#| echo: false
#| eval: true
#| cache: true
#| warning: false
...
-   $W_i$ is the mapped proportion of area of class $i$
...
Therefore, the total number of samples depends on the assumptions about the user's accuracies $U_i$ and the expected standard error $S(\hat{O})$. Once the sample size is estimated, there are several methods for allocating samples per class [@Olofsson2014]. One option is proportional allocation, in which the sample size in each stratum is proportional to the stratum’s size in the population. In land use mapping, some classes often have small areas compared to the more frequent ones. Using proportional allocation, rare classes will have small sample sizes, decreasing the accuracy of their estimates. Another option is equal allocation, in which all classes have the same number of samples; however, equal allocation may fail to capture the natural variation of classes with large areas.

As alternatives to proportional and equal allocation, Olofsson et al. [@Olofsson2014] suggest ad-hoc approaches where each class is assigned a minimum number of samples. They propose three allocations where 50, 75, and 100 sample units are allocated to the less common classes, with proportional allocation used for the more frequent ones. These allocation methods should be considered as suggestions, and users should be flexible to select alternative sampling designs.
...
The next step is to choose one of the options for sampling design to generate a set of points for stratified sampling. These points can then be used for accuracy assessment. This is achieved by function `sits_stratified_sampling()` which takes the following parameters:
...
In the example below, we choose the "alloc_120" option from the sampling design to generate a set of stratified samples. The output of the function is an `sf` object with points that include their location (latitude and longitude) and the class assigned in the map. We can also generate a SHP file with the sample information. The script below shows how to use `sits_stratified_sampling()` and also how to convert an `sf` object to a SHP file.

:::{.panel-tabset}
## R
```{r}
#| label: ro-stratified-sampling
#| echo: true
#| eval: true
#| cache: true
# Generate a set of stratified samples for measuring area accuracy
...
# The output of sits_stratified_samples is an `sf` object
...
Using the shapefile, users can visualize the points in a standard GIS such as QGIS. For each point, they will indicate the correct class. In this way, they will obtain a confusion matrix which will be used for accuracy assessment. The `overhead` parameter allows users to discard border or doubtful pixels where the interpreter cannot be confident of the class assignment. By discarding points whose attribution is uncertain, users improve the quality of the assessment.

After all sampling points are labelled in QGIS (or similar), users should produce a CSV file, a SHP file, a data frame, or an `sf` object, with at least three columns: `latitude`, `longitude` and `label`. See the next section for an example on how to use this data set for accuracy assessment.
...
Adjusting for area size allows us to produce an unbiased estimate of the total area of class $j$, defined as a stratified estimator 
$$
...
-   `validation`: a CSV file, SHP file, GPKG file, `sf` object, or data frame containing at least three columns (`latitude`, `longitude`, and `label`), with a set of well-selected labeled points obtained from the samples suggested by `sits_stratified_sampling()`.

In the example below, we use a validation set produced by the researchers who produced the Rondonia data set, described above. The validation team used QGIS to produce a CSV file with validation data, which is then used to assess the area accuracy using the best practices recommended by [@Olofsson2014]. 
...
area_accuracy <- sits_accuracy(
    rondonia_2022_reclass, 
...
The results show a clear distinction between forest areas and those associated with deforestation (`Clear_Cut`). It also shows a possible source of confusion between wetlands and clear-cut areas, suggesting that additional training samples for those classes could improve performance.

## Summary

This chapter provides an example of the recommended statistical methods for designing stratified samples for accuracy assessment. However, these sampling methods depend on perfect or near-perfect validation by end-users. Ensuring best practices in accuracy assessment requires both a well-designed sample and consistent interpretation of reference labels that align with the classes used to train the classifier.
