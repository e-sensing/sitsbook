...
The premise of the "time-first" is the temporal profile carries most of the discriminative information for distinguishing land classes (crop types, pasture vs. natural vegetation, forest vs. degraded forest). Space is used afterwards to capture neighborhood relations. Algorithms are trained on labelled time-series samples, where each time series is treated independently. Time series-based methods benefit from traditional practices in Remote Sensing, which use point-based locations to provide labels for machine learning algorithms.

The "space-first, time-later" approach inherits practices from Computer Vision used for photo and video classification. Its methods use a small window of pixels (a "patch," e.g., 16×16, 32×32, or 64×64) as the classification unit. The patch provides the model with spatial context — texture, shape, neighbourhood arrangement — that pure pixel-based methods lack. It's the input format for convolutional neural networks (CNNs) [@Song2019a] semantic segmentation [@Garnot2022], and vision transformers [@Tarasiou2023]. These methods are inherited from Computer Vision, where the main aim of image classification is to describe objects in photographs, based on a ``foreground vs. background" paradigm.

Time-first is best suited to mapping phenological changes in continuous fields such as agriculture and forests. Space-first is more appropriate for detecting spatial patterns in urban morphology, fine field boundaries, and object-level detail. In time-first, space is a smoothing step decoupled from classification. In space-first, spatial context is part of the learned features.

Two key differences between these approaches are the availability of training data and the computational demand. Time-first works with point/pixel samples, which are what field campaigns and reference databases naturally produce. Space-first needs spatially contiguous labelled patches, which are costlier to obtain. Furthermore, time-first classifies data in parallel. Each pixel is independent, so algorithms scale to continental data sets, and sometimes don't need GPUs (Random Forest works well). Space-first methods are computationally demanding and require GPUs, which can limit their practical application in large-scale studies. 

The tradition of Remote Sensing experts is to produce labelled training samples by identifying individual pixels or polygons in images. This approach is easily extendable to image time series labelling. It is much less common for experts to provide labelled patches, which are required by many classification algorithms such as 2D semantic segmentation and convolutional neural networks [@Ajibola2024].
