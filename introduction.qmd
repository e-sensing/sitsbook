...
Satellite imagery provides the most extensive data on our environment. By encompassing vast areas of the Earth's surface, satellite images enable researchers to analyze local and worldwide transformations. By observing the same location multiple times, satellites provide data on environmental changes and survey areas that are difficult to observe from the ground. Given these unique features, satellite images offer essential information for many applications, including monitoring deforestation, crop production, food security, urban footprints, water scarcity, and land degradation. Using time series, experts improve their understanding of ecological patterns and processes. Instead of selecting individual images from specific dates and comparing them, researchers track change continuously [@Woodcock2020].

## Time-first, space-later

"Time-first, space-later" is an approach to satellite image classification that treats time series analysis as the first step in analyzing remote sensing data, bringing in spatial information only afterwards. Typically, a one- or two-year series of Sentinel-2 multispectral data is used, taken every 15/16 days. In the *time-first* phase, classification is pixel-based: a model assigns a label to each pixel from its temporal profile, producing a set of labeled pixels. This focus on time helps to identify landscape change, detecting and tracking seasonal and long-term trends and flagging anomalous events such as wildfires, floods, or droughts. In the *space-later* phase, a smoothing algorithm refines these results by considering the spatial neighborhood of each pixel. The resulting map therefore combines both temporal and spatial information.
...
The two approaches differ most in what they are suited for, the training data they require, and their computational cost. Time-first is best for mapping phenological change in continuous fields such as agriculture and forests, where space acts as a smoothing step decoupled from classification. Space-first is better for detecting spatial patterns in urban morphology, fine field boundaries, and object-level detail, where spatial context is part of the learned features. Time-first works with point and pixel samples, which  are what field campaigns and reference databases naturally produce. Since pixels are classified independently, the methods scales to continental data sets and can run without GPUs (Random Forest works well). Space-first needs spatially contiguous labeled patches, which are costlier to obtain, and it is computationally demanding, typically requiring GPUs; this can limit its use in large-scale studies.
...
4. Given a set of ground truth values in formats such as CSV or ESRI Shapefile and a regular data cube, use `sits_get_data()` to obtain training samples containing time series for selected locations in the training area.
...
7. Refine the class probabilities using Bayesian smoothing over each pixel's spatial neighborhood with `sits_smooth()`.
...
6.  Generating and classifying embeddings using self-supervised learning.
