...
4. Classification: After the image has been partitioned into distinct objects, the next step is to classify the image using the model produced in the previous step. This step uses `sits_classify()`, which performs a pixel-based classification to obtain a set of probabilities. The segments are not used in the classification. They are saved for the next two steps: smoothing and labeling. Pixel-based classification is much faster than polygon-based classification and the results are the same.

5. Smoothing: Smoothing vector cubes use a different definition of neighborhood than  [raster-based probability smoothing](https://e-sensing.github.io/sitsbook/cl_smoothing.html). When dealing with raster cubes, neighborhoods are defined based on rectangular windows (e.g., 9 x 9 size). When dealing with vector cube, neighborhoods are defined by the segments. In this case, all pixels inside a segment are considered part of the same neighborhood. When estimating the spatial effects, the probabilities of all pixels in the segment are used to calculate the smoothed probability of each pixel. 

5. Map labelling: Once the set of probabilities have been obtained for selected time series inside a segment, they can be used for labeling. To label the segments, `sits` takes the probabilities assigned to each pixel inside a segment and uses an aggregation function to calculate the estimaed value of class probabilities for each segment. The aggregation functions available include `mean` or `median`. After the aggregation, `sits` selected the label of each segment based on maxium aggregate value for the class probabilities. This step uses `sits_label_classification()`, as shown below.
...
| `gpu_memory` | GPU memory available for classification if a GPU is available (default = 4)) | 
| `batch_size` | size of batches used for processing (default = 1000*`gpu_memory`) |
...
Then we can compute the smoothed probability cube. 
