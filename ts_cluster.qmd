...
<a href="https://www.kaggle.com/code/esensing/hierarchical-clustering-of-time-series" target="_blank">
...
## Python
```{python}
#| label: py-samples-cerrado-2-classes
#| warning: false
#| cache: true
# Show the summary of the cerrado_2_classes dataset
summary(cerrado_2classes)
...
The `linkage` parameter defines the distance metric between clusters. The recommended linkage criteria are: `complete` or `ward.D2`. Complete linkage prioritizes the within-cluster dissimilarities, producing compact clusters (samples close together), but results are sensitive to outliers. As an alternative, Ward proposes to use the sum-of-squares error to minimize data variance [@Ward1963]; his method is available as the `ward.D2` option to the `linkage` parameter. To cut the dendrogram, the `sits_cluster_dendro()` function computes the adjusted rand index (ARI) [@Rand1971], returning the height where the cut of the dendrogram maximizes the index. In the example, the ARI index indicates that there are six clusters. The result of `sits_cluster_dendro()` is a time series tibble with one additional column called "cluster". The function `sits_cluster_frequency()` provides information on the composition of each cluster.

:::{.panel-tabset}
## R
```{r}
#| label: samples-cerrado-clusters
#| warning: false
#| cache: true
...
## Python
```{python}
#| label: py-samples-cerrado-clusters
#| warning: false
#| cache: true
# Show clusters samples frequency
sits_cluster_frequency(clusters)
...
## Python
```{python}
#| label: py-samples-cerrado-clean
#| warning: false
#| cache: true
# Remove cluster 3 from the samples
...
In this chapter, we present hierarchical clustering to improve the quality of training data. This method works well for up to four classes. Because of its quadratic computational complexity, it is not practical to use it for data sets with many classes. In this case, we suggest the use of self-organized maps (SOM) as shown in the next chapter.
