...
The term "foundation model" entered the AI lexicon around 2021, coined by researchers at Stanford to describe large neural networks trained on vast, diverse datasets that can be adapted — through fine-tuning or prompting — to a wide range of downstream tasks. GPT-4, Gemini, and Claude are foundation models for language. Their defining characteristic is not size alone, but the breadth of their pretraining: they learn general representations of their domain that transfer efficiently to specialized problems. 

Earth observation foundation models (EOFMs) apply this logic to satellite and aerial imagery. Rather than training a separate model from scratch for each task, such as flood detection or crop classification, an EO foundation model attempts to produce general-purpose representations. Their assumption is that such shared backbones can then be adapted to downstream applications with relatively little labeled data.
...
In `sits` we provide five methods for building embeddings. Three are self-supervised:  (a) Masked Autoencoders (MAE); (b) Variance-Invariance-Correlation regularization (VICReg); (c) Lean Joint-Embedding Predictive Architectures (LeJEPA). Three are supervised: (a) Barlow Twins; (b) Supervised Contrastive Learning.
...
The MAE loss function is masked mean squared error, computed only over masked positions.
With binary mask $m_{i,t} \in \{0,1\}$ (1 = masked), reconstruction $\hat{x}$ and
normalized target $x$:
...
The denominator is clamped to at least 1 to avoid division by zero. Visible
timesteps contribute nothing to the loss.
...
The embeddings produce two resampling-augmented views per sample. Let $Z^A, Z^B \in \mathbb{R}^{N \times D}$ be the projector outputs of the two views over a batch of size $N$ ($D = d_p$), where $D$ is the number of dimensions of the input. The VICReg loss sums three terms: 
...
with $V = 2$, where $N$ is the batch size and $D$ the dimension of the projector head. It has two components: the invariance loss and the SIGReg one.
...
distribution toward an isotropic Gaussian. It uses two parameters: (a) the number ($D$) of random projection directions (`num_slices`); and (b) the number $K$ of quadrature knots for the characteristic function tests (`num_knots`). 

For each of $S$ random unit
directions $a \in \mathbb{R}^D$ (`num_slices`),  it forms the scalar projections
$u_i = z^{(v)}_i \cdot a$ and compares their empirical characteristic function
(ECF) to the standard-Gaussian characteristic function $\phi(t) = e^{-t^2/2}$,
using an Epps–Pulley statistic evaluated at $K$ quadrature knots
$t_k \in [0, 3]$ (`num_knots`):
...
where $\tilde w_k$ are trapezoidal quadrature weights times $\phi(t_k)$. The two terms combine with a single trade-off:
...
The loss function works by taking a batch of $N$ view-pairs. Run both views through encoder + projector to get $Z^A, Z^B \in \mathbb{R}^{N \times D}$, where $D$ is the dimension of the projector. Standardize each feature across the
batch (mean 0, unit variance), then form the empirical cross-correlation
matrix:
...
The objective function drives $C$ toward the identity matrix:
...
The invariance term pushes the diagonal to 1, so the same feature computed from the two same-class views should be perfectly correlated. The redundancy-reduction term pushes the off-diagonals to 0: distinct mbedding dimensions should be decorrelated, so they carry non-overlapping information.
...
#|  A shared encoder f_0 maps each series to an embedding. The SupCon loss attracts embeddings
