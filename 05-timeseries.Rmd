...
To get a time series in sits, one has to create a data cube, as described previously.  Users can request one or more time series points from a data cube by using `sits_get_data()`. This function provides a general means of access to image time series using the `samples` parameter. This parameter accepts the following data types: 
...
In the example below, given a data cube the user provides the latitude and longitude of the desired location, the bands. Since the  start date and end date of the time series are not informed, `sits` obtains them from the data cube. The result is a tibble with one time series that can be visualized using `plot()`.
...
# first, create a tibble with latitude and longitude information
...
Users can also specify samples by providing shapefiles or `sf` objects in point or polygon format. In this case, the geographical location is inferred from the geometries associated with the shapefile. For files containing points, the geographical location is obtained directly. For polygon shapes, the parameter `n_sam_pol` (defaults to 20) determines the number of samples to be extracted from each polygon. The temporal information can be provided explicitly by the user; if absent, it is inferred from the data cube from which the samples are extracted. If label information is available in the shapefile, users should include the parameter `label_attr` to indicate the name of the column which contains the label to be associated with each time series.

```{r, warning = FALSE, message = FALSE}
# obtain a set of points inside the state of Rondonia, Brazil
# provide a shapefile with the state boundaries
shp_file <- system.file("extdata/shapefiles/rondonia/rondonia.shp", 
                        package = "sits")
...
# create a data cube based on data stored in the Microsoft Planetary Computer
# select a region of interest that corresponds to the state of Rondonia
...
# read the points in the shapefile and produce a tibble with time series
# users can replace the shapefile with the equivalent sf object. 
