...
```{r api, echo = FALSE, out.width = "100%", out.height = "100%", fig.align="center", fig.cap="Diagram of a random decision forest (source: Venkata Jagannath in Wikipedia)."}

knitr::include_graphics("images/random_forest.png") 
```

SITS provides a `sits_rfor` function which is a front-end to the `randomForest` package[@Wright2017]; its main parameter is `num_trees` (number of trees to grow). In practice, about 100 trees are sufficient to achieve a reasonable classification result. The model can be visualised using `plot()`
...
Random forests are robust to outliers and able to deal with irrelevant inputs [@Hastie2009]. However, despite being robust, this approach tends to overemphasize some variables and thus rarely turns out to be the classifier with the smallest error. One reason is that the performance of random forests tends to stabilize after a part of the trees are grown [@Hastie2009]. In cases where abrupt change takes places, such deforestation mapping, random forest (if properly trained) will emphasize the temporal instances and bands that capture such quick change. Random forests can be useful to provide a baseline to compare with more sophisticated methods. 
