...
Welcome to the age of big Earth observation data! With free access to massive data sets, we need new methods to measure change on our planet. This book will help you use state-of-the-art tools to work with image time series. Combined with Earth observation data cubes, time series are a powerful tool for monitoring change, providing insights and information that single snapshots cannot. Time series analysis is a new and exciting paradigm, and this book offers a comprehensive introduction to this emerging discipline.
:::

## What is this book about? {.unnumbered}
...
The `sits` API includes training sample quality measures, machine learning and deep learning classification algorithms, and Bayesian post-processing methods for smoothing and uncertainty assessment. The package also supports object-based spatio-temporal segmentation for space-time OBIA. To evaluate results, `sits` supports best-practice accuracy assessments.

`sits` can generate embeddings using self-supervised learning with masked autoencoders, LeJEPA (Lean Joint-Embedding Predictive Architecture), and VICReg (Variance-Invariance-Covariance Regularization), as well as supervised training for embedding generation with Barlow Twins and Supervised Contrastive Learning.

The authors also provide a **Python** API that interfaces with the **R** API, and thus allows Python users to directly run `sits` and convert its data structures to Python `DataFrames` and `xarrays`.
...
- Gilberto Camara is a Senior Research Fellow at Brazil's National Institute for Space Research (INPE).
...
- Bianca Zadrosny is a Professor at Catholic University of Rio de Janeiro (PUC-RIO).

## How much R knowledge is required? {.unnumbered}

To quickly master what is needed to run `sits` in R, please read Parts 1 and 2 of Garrett Grolemund’s book, [Hands-On Programming with R](https://rstudio-education.github.io/hopr/). Although not needed to run `sits`, your **R** skills will benefit from the book by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund, [R for Data Science (2nd edition)](https://r4ds.hadley.nz/). Important concepts of spatial analysis are presented by Edzer Pebesma and Roger Bivand in their book [Spatial Data Science](https://r-spatial.org/book/).
...
The version of the `sits` package described in this book is 2.0.0.

## API documentation {.unnumbered}
