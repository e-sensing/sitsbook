...
### Accuracy measures for classified images{-}
...
In what follows, we show an example of using the accuracy function to estimate the quality of the classification. We will be using a classified data cube that contain labelled images which are part of result of Landsat-8 classification for the Brazilian Cerrado biome, as described in the paper by Simoes et al. [@Simoes2021]. 
...
To do the assessment, Simoes et al.[Simoes2021] merged the labels "Cerrado" and "Open Cerrado" into one label called "Cerrado". 



The files cover tiles "042049", "042050", "043049", "043050" from the Brazil Data Cube. In what follows, we create the classified data cube.

```{r, tidy="styler"}
# define the directory where data is located
data_dir <- system.file("extdata/Cerrado", package = "sitsdata")
# create a data cube with the classified images
cerrado_classif <- sits_cube(
    source = "BDC", 
    collection  = "LC8_30_16D_STK-1",
    data_dir = data_dir,
    bands = "class",
    labels = c("Annual_Crop", "Cerradao", "Cerrado", 
                "Nat_NonVeg", "Cerrado", "Pasture", 
                "Perennial_Crop", "Silviculture", 
                "Sugarcane", "Water"), 
    parse_info = c("X1", "tile", "band", 
                   "start_date", "end_date", "version")
)
# plot the classification
plot(cerrado_classif)
```
After creating the classified cube, Simoes et al.[Simoes2021] use a set of validation points to obtain an accuracy assessment of the classification. These points come from  a systematic sampling of the Cerrado biome using a 20 km2 grid with a total of 5402 points. These samples are independent of the training set used in the classification and were interpreted by five specialists using high resolution images from the same period of the classification. The authors also did additional sampling to reach a minimal number of samples for the classes "Natural Non Vegetated", "Perennial Crop", and "Water". This resulted in 5286 evaluation samples thus distributed: "Annual Crop"(553), "Cerradao" (704), "Cerrado" (2451), "Natural Non Vegetated" (44), "Pasture" (1246), "Perennial Crop" (38), "Silviculture" (94), "Sugarcane" (77), and "Water" (79). The assessment is done by `sits_accuracy()`. 
