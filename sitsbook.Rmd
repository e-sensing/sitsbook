...
## Accessing Image Time Series Services{-} 

The `sits` package also supports two services that allow access to individual time series: (a) WTSS (Web Time Series Service), provided by the Brazil Data Cube; (b) SATVEG, provided by the Center for Agricultural Informatics of Brazil's Agricultural Research Agency(EMBRAPA). The WTSS services allows access to all image collections supported by the BDC. The following example shows how to create a data cube associated with the WTSS service, and how to access time series from it. More details on accessing time series are provided in [Chapter  3](https://e-sensing.github.io/sitsbook/acessing-time-series-information-in-sits.html). Users need a BDC access key to access the WTSS service.

```{r, eval = FALSE}
# enter BDC access credentials 
Sys.setenv(
    "BDC_ACCESS_KEY" = <your_bdc_access_key>
)
# create a data cube to access time series from the MOD13Q1-6 collection available in the Brazil Data Cube
cube_wtss <- sits_cube(
    source = "WTSS",
    collection = "MOD13Q1-6"
    
)
# access a time series from the MOD13Q1-6 collection
point_wtss <- sits_get_data(
    cube = cube_wtss,
    longitude = -55.50563,
    latitude  = -11.71557,
...
The SATVEG service provides access to three collections obtained by the MODIS sensor: (a) "TERRA", which gives access NDVI and EVI to the MOD13Q1-6 collection from the NASA Terra satellite at 250m resolutionin 16 day intervals; (b) "AQUA", which contains time series from the the MYD13Q1-6 from the NASA Aqua satellite with the same resolution; (c) "COMB", which combines time series from both sensors for improved temporal resolution. Access to SATVEG is illustrated below.
...
```{r}
# files are named using the convention 
# "Sentinel-2_Rondonia_P16D_rfor_20L_2020-06-01_2021-08-31_class.tif"
Rondonia_class_cube <- sits_cube(
      ...
      delim = "_",
      parse_info = c("X1", "X2", "X3", "X4", "tile", "start_date", "end_date", "band")
)
# show the timeline of the cube
sits_timeline(cbers_cube)
```
...
When joining different images to get the best image for a period, `sits_regularize()` uses an aggregation method, defined by the parameter `agg_method`. It specifies how individual values of different pixels should be combined. The default is `least_cc_first`, which organizes the images for the chosen interval in order of increasing cloud cover, and selects the first cloud-free pixel in the sequence. For more details, see `?sits_regularize()`.
...
<!--chapter:end:10-casestudies.Rmd-->
