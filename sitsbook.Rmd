...
5. To run the examples in the book, please also install `sitsdata` package, which is available from GitHub. It is necessary to use package `devtools` to install `sitsdata`.
...
```{r, echo = FALSE, out.width = "90%", fig.align="center", fig.cap="Flow chart of XGBoost algorithm (Source: Guo et al.,Applied Sciences, 2020. - licenced as CC-BY-SA 4.0)."}
...
# Plot the cube
plot(rondonia_20LMR, date = "2022-07-16", red = "B11", green = "B8A", blue = "B02")
```
...
The next step is to train a machine learning model to illustrate CPU-based classification a random forest model and a tempCNN model. We build a random forest model using `sits_train()` and then create a plot to find out what are the most important variables for the model.
...
plot(rfor_model)
```
The figure shows that B11 band values in date 15 ("2022-08-17") is the most informative for the random forest model, followed by EVI index values in date 9 ("2022-05-13"). These bands and dates represent inflection points in the image time series. 

## Classification of machine learning models in CPUs{-}

<!--
PEDRO:
achei essa frase abaixo confusa. as funcoes usam processamento paralelo mas é
o sits que faz isto.
-->
...
```{r, tidy = "styler", out.width = "100%", fig.align="center", fig.cap="Smoothened probabilities for class Forest"}
...
plot(rondonia_20LMR_class, tmap_options = 
         list("scale" = 0.7))
```
...
```{r, tidy="styler", out.width = "100%", fig.align="center", fig.cap="Final map of deforestation obtained using TempCNN model (Source: Authors)."}
...
```{r, echo = FALSE, out.width="90%", fig.caption = "Detail of labeled segements for image in Rondonia, Brazil", fig.align="center"}
...
```{r, tidy = "styler", out.width = "100%", out.height = "80%", fig.align="center", fig.cap="Example of defining colors for the Anderson Land Classification Scheme(Source: Authors)."}
