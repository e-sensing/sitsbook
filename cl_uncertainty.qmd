...
<a href="https://www.kaggle.com/code/esensing/uncertainty-and-active-learning" target="_blank">
...
In traditional classification methods, experts provide a set of training samples and use a machine learning algorithm to produce a map. By contrast, the active learning approach puts the human in the loop [@Monarch2021]. At each iteration, an unlabeled set of samples is presented to the user, who assigns classes to them and includes them in the training set [@Crawford2013]. The process is repeated until the expert is satisfied with the result, as shown in @fig-uncert-1.
...
* *Data limitations* : The quality and quantity of input data can influence the accuracy of the classification results.
...
The machine learning literature [@Hullermeier2021; @Gawlikowski2023] distinguishes between two broad types of uncertainty:

* *Data uncertainty* : also called *aleatoric uncertainty*, it refers to irreducible noise inherent in the observation process. It results from the inherent variability of the landscape, the discrete nature of remote sensing spatial and spectral resolution, and natural confusion about the land use and land cover classes to be measured. This kind of uncertainty is external to the classification processing. Many authors [@Hullermeier2021; @Gawlikowski2023] argue this is an irreducible component of the measurement process.

* *Epistemic uncertainty* : also called *model uncertainty* results from limitations of the model to provide a good quality classification. This can be caused either due to lack of adequate training data, limited capacity of the model, or both. Improving the training data or choosing a better algorithm reduces this kind of uncertainty.

The function `sits_uncertainty()` calculates an uncertainty cube based on the probabilities produced by the classifier, aiming to capture epistemic uncertainty. It takes a probability cube as input. The uncertainty measure is relevant in the context of active learning. It helps to increase the quantity and quality of training samples by providing information about the model's confidence. The supported types of uncertainty are 'entropy', 'least', 'margin', and 'ratio'.
...
*Ratio of confidence* is the measure of the ratio between the two most confident predictions, expressed in a range from 0% (no uncertainty) to 100% (maximum uncertainty). Let $P_1(i)$ and $P_2(i)$ be the two higher class probabilities for pixel $i$. Then, the ratio of confidence is expressed as
$$
...
# Select a small area outside the tile
...
The next step is to classify this study area using a training set with 480 time series collected over the state of Rondonia (Brazil) for detecting deforestation. The training set uses 4 classes (`Burned_Area`, `Forest`, `Highly_Degraded`, and `Cleared_Area`). The cube is classified using a Random Forest model, post-processed by Bayesian smoothing, and then labeled.
...
plot(s2_cube_label, label_position = "outside")
```
## Python
```{python}
#| eval: false
...
# Load the training set
samples_prodes_4classes = load_samples(
...
```{r}
#| echo: false
#| label: py-fig-uncert-cube-class1
#| cache: true
#| fig-width: 7
#| fig-height: 4
#| fig-dpi: 300
...
```{r}
#| echo: false
#| label: py-fig-uncert-cube-margin
#| cache: true
#| fig-width: 7
#| fig-height: 4
#| fig-dpi: 300
...
As expected, the places of highest uncertainty are those covered by surface water or associated with wetlands. These places are likely to be misclassified. For this reason, `sits` provides `sits_uncertainty_sampling()`, which takes the uncertainty cube as its input and produces a tibble with locations in WGS84 with high uncertainty. The function has three parameters: `n`, the number of uncertain points to be included; `min_uncert`, the minimum value of uncertainty for pixels to be included in the list; and `sampling_window`, which defines a window where only one sample will be selected. The aim of `sampling_window` is to improve the spatial distribution of the new samples by avoiding the inclusion of points in the same neighborhood. After running the function, we can use `sits_view()` to visualize the location of the samples.

:::{.panel-tabset}
## R
```{r}
#| label: uncert-rondonia20LMR-sampling
#| cache: true
#| warning: false
#| result: false
...
The visualization shows that the samples are located in the areas covered by the Samuel dam. Thus, we designate these samples as `Wetland`. A more detailed evaluation, which is recommended in practice, requires analyzing these samples with exploration software such as QGIS and individually labeling each sample. In our case, we will take a direct approach for illustration purposes.
...
plot(s2_cube_label_v2, label_position = "outside")
```
## Python
```{python}
#| eval: false
...
```{r}
#| echo: false
#| label: py-fig-uncert-cube-class2
#| cache: true
#| fig-width: 7
#| fig-height: 4
#| fig-dpi: 300
...
The results show a significant quality gain over the earlier classification. There are still some areas of confusion in the exposed soils outside the inundation area, some of which have been classified as burned areas. It is also useful to show the uncertainty map associated with the second model.
...
As the new uncertainty map shows, there is a significant improvement in the quality of the classification. The remaining areas of high uncertainty are those affected by the contrast between the wet and dry seasons close to the inundation area. These areas are low-lying places that sometimes are covered by water and sometimes are bare soil areas throughout the year, depending on the intensity of the rainy season. To further improve the classification quality, we could obtain new samples of those uncertain areas, label them, and add them to samples.

## Summary

In general, as this chapter shows, combining uncertainty measurements with active learning is a recommended practice for improving classification results. This approach offers several important benefits for remote sensing image classification, especially in contexts where obtaining high-quality labeled data is expensive and time-consuming. Uncertainty maps direct experts to places where new samples are required. In this way, combining uncertainty with active learning minimizes the number of samples that need to be manually labeled by selecting the most informative ones.
