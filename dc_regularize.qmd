...
2. Temporal harmonisation: creates equispaced intervals (e.g., 16-day, monthly or seasonal composites) and fills gaps due to cloud cover and sensor errors. In `sits`, all images inside a chosen interval are combined by stacking. Images are ordered by decreasing percentage of cloud cover. The least-cloud filled image is taken as reference, and the other images are used to try to fill ist gaps. Pixels with persistent cloud-cover are marked as "NA" and are temporally interpolated during computation. 
...
```{r}
#| eval: false
# plot the least cloudy image of the irregular cube
s2_cube_rondonia |>  
...
#| out-width: 100%
#| fig-cap: |
#|   Regularized image for tile Sentinel-2 tile 20LLP.
#| fig-align: center
# Regularize the cube to 16-day intervals
reg_cube_rondonia <- sits_regularize(
          cube       = s2_cube_rondonia,
          output_dir = "./tempdir/R/datacubes",
          res        = 40,
          period     = "P16D",
          multicores = 6)
...
if (!file.exists("./tempdir/R/datacubes/sar"))
    dir.create("./tempdir/R/datacubes/sar")
...
cube_s1_reg <- sits_regularize(
    ...
    memsize = 12,
    multicores = 6,
    output_dir = "./tempdir/R/datacubes/sar"
)
...
Images from the HLS Landsat and Sentinel-2 collections are accessed separately and can be combined with `sits_merge()`. We first create tow ARD collections, one for HLS Sentinel-2 and one for HLS Landsat over the same area. The two cubes are then merged. 
...
```
Although The merged cube contains a tenser timeline. However, for further use in `sits`, we need to create a regularized cube to obtain a less cloud covered composite. 
```{r}
